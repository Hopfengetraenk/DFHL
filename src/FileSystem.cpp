...
File::File(WIN32_FIND_DATA fileInfo, Path* newParent) {
	...
	size = fileInfo.nFileSizeLow + ((INT64)MAXDWORD + 1) * fileInfo.nFileSizeHigh;
...
void File::setHash(INT64 newHash) {
	hashAvailable = true;
	hash = newHash;
}
...
Collection* FileSystem::getFolderContent(Path* folder) {
	...
	if (hFind == INVALID_HANDLE_VALUE) {
		...
		//Statistics::getInstance()->dirOpenProblems++;
	} else {
		do {
...
bool FileSystem::hardLinkFiles(File* file1, File* file2) {
	...
	logInfo(L"Linking %s and %s", file1Name, file2Name);

	// Step 1: precaution - rename original file
	wsprintf(file2Backup, L"%s.backup", file2Name);
